...
class Resize(object):
    ...
    def __init__(self,
        ...
        self.grasp_clip_border = grasp_clip_border
    ...
    def _resize_grasps(self, results):
        for key in results.get('grasp_fields', []):
            scale_factor = results['scale_factor']
            scale_factor = np.tile(
                (scale_factor[0], scale_factor[1]), 4).astype(results[key].dtype)
            grasps = results[key] * scale_factor
            if self.grasp_clip_border:
                img_shape = results['img_shape']
                grasps[:, 0::2] = np.clip(grasps[:, 0::2], 0, img_shape[1])
                grasps[:, 1::2] = np.clip(grasps[:, 1::2], 0, img_shape[0])
            results[key] = grasps
...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            # flip grasps
            for key in results.get('grasp_fields', []):
                results[key] = self.grasp_flip(results[key],
                                               results['img_shape'],
                                               results['flip_direction'])
            # flip masks
            for key in results.get('mask_fields', []):
...
class Expand(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('grasp_fields', []):
            results[key] = results[key] + np.tile(
                (left, top), 4).astype(results['gt_grasps'].dtype)
