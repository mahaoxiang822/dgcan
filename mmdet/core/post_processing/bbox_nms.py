...
def multiclass_poly_nms_8_points_graspnet(multi_bboxes,
                        multi_scores,
                        multi_scores_reg,
                        nms_cfg,
                        score_thr=0,
    ...
    for i in range(num_classes - 1):
        ...
        _scores = multi_scores[cls_inds, i]
        _scores_reg = multi_scores_reg[cls_inds, :]

        cls_dets = torch.cat([_bboxes, _scores[:, None]], dim=1)
        cls_dets, ind = nms_op(cls_dets, **nms_cfg_)
        cls_dets = torch.cat([cls_dets, _scores_reg[ind]], dim=1)
        cls_labels = multi_bboxes.new_full((cls_dets.shape[0],),
                                           i,
                                           dtype=torch.long)
        bboxes.append(cls_dets)
