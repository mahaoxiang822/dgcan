...
class MaxScoreAssignerGraspNet(BaseAssigner):
    ...
    def __init__(self,
        ...
        self.assign_gt_labels = assign_gt_labels

    def assign(self, bboxes, gt_bboxes, gt_scores, gt_bboxes_ignore=None, gt_labels=None, domain='source'):
        """Assign gt to bboxes.
        ...
        if domain == 'target':
            ...
            assigned_gt_inds = bboxes.new_zeros((bboxes.shape[0], ), dtype=torch.int64)
            assigned_gt_inds[:int(0.25 * bboxes.shape[0])] = 1
            max_overlaps = bboxes.new_zeros((bboxes.shape[0], ), dtype=torch.float)
            max_overlaps[:int(0.25 * bboxes.shape[0])] = 1.0
            assigned_labels = bboxes.new_full((bboxes.shape[0], ), -1, dtype=torch.float)
            assigned_labels[:int(0.25 * bboxes.shape[0])] = 0
            return AssignResult(
