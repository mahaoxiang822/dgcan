...
class DeltaXYWHSinCosGraspCoder(BaseBBoxCoder):
...
def xywhtheta_to_delta(rois, gts, means=[0, 0, 0, 0, 0, 0], stds=[1, 1, 1, 1, 1, 1]):
    x = rois[:, 0]
    y = rois[:, 1]
    w = rois[:, 2]
    h = rois[:, 3]
    ...
    deltas = torch.stack([delta_x, delta_y, delta_w, delta_h,
                          delta_sin2theta, delta_cos2theta], dim=-1)
...
def delta_to_xywhtheta(deltas,
    ...
    delta_x = deform_deltas[:, 0::6]
    delta_y = deform_deltas[:, 1::6]
    delta_w = deform_deltas[:, 2::6]
    delta_h = deform_deltas[:, 3::6]
    delta_sin2theta = deform_deltas[:, 4::6]
    delta_cos2theta = deform_deltas[:, 5::6]
    ...
    theta = (roi_recs[:, 4]).unsqueeze(1).expand_as(delta_sin2theta)
    ...
    return torch.stack([x_pred, y_pred, w_pred, h_pred, theta_pred], dim=-1).view(-1, 5)
