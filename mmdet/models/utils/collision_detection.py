...
from graspnetAPI.utils.utils import get_camera_intrinsic

def depth_to_pcd(depth, camera):
    camera_intr = get_camera_intrinsic(camera)
    h, w = depth.shape
    pcds = []
    fx = camera_intr[0][0]
    fy = camera_intr[1][1]
...
class ModelFreeCollisionDetector():
    ...
    def __init__(self, depths, camera, voxel_size=0.005):
        self.finger_width = 0.01
        self.finger_length = 0.06
        self.voxel_size = voxel_size
        scene_points = depth_to_pcd(depths, camera)
