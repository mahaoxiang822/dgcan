from graspnetAPI import GraspNetEval, GraspGroup, RectGraspGroup
    ...
    def eval_scene(self, scene_id, dump_folder, TOP_K=50, return_list=False, vis=False, max_width=0.1):
        ...
        **Output:**
        ...
        for i in tqdm(range(len(annIds)), 'Scene:{}'.format(scene_id)):
            ...
            depth = cv2.imread(os.path.join(self.root, 'depths', 'scene_%04d' % scene_id, self.camera, "depth_fill_hole_bilateral_outlier", "%04d.png" % ann_id), cv2.IMREAD_UNCHANGED)
            grasp_group = rect_labels.to_grasp_group(self.camera, depths=depth)
            # 否则，直接加载
...
# graspnet_root = r'G:\dataset\planer_graspnet'
ge = MyGraspNetEval(root=graspnet_root, camera='kinect', split='test', view='1016')
dump_folder = os.path.join('/home/qrrr', 'rectangle-graspnet-multiObject-multiGrasp', 'grasp_multiObject_multiGrasp', 'tools', 'predicted_rectangle_grasp')
res, ap = ge.eval_seen(dump_folder, proc=6)
print("AP_seen: %f" % ap[0])
print("AP0.4_seen: %f" % ap[1])
print("AP0.8_seen: %f" % ap[2])
res, ap = ge.eval_similar(dump_folder, proc=6)
print("AP_similar: %f" % ap[0])
print("AP0.4_similar: %f" % ap[1])
print("AP0.8_similar: %f" % ap[2])
res, ap = ge.eval_novel(dump_folder, proc=6)
